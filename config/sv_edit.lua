...
-- Universal revive function compatible with multiple ambulance scripts
local function triggerRevive(targetId)
    -- Wasabi Ambulance (ESX/QB)
    if isStarted('wasabi_ambulance') then
        TriggerClientEvent('wasabi_ambulance:revive', targetId)
        return
    end
    
    -- Brutal Ambulance
    if isStarted('brutal_ambulance') then
        TriggerClientEvent('brutal_ambulance:revive', targetId)
        return
    end
    
    -- PS Ambulance / cd_dispatch
    if isStarted('ps-adminmenu') or isStarted('cd_dispatch') then
        TriggerClientEvent('ps-adminmenu:client:Revive', targetId)
        return
    end
    
    -- QBX Core
    if isStarted('qbx_medical') then
        TriggerClientEvent('qbx_medical:client:Revive', targetId)
        return
    end
    
    -- QB Ambulancejob
    if isStarted('qb-ambulancejob') then
        TriggerClientEvent('hospital:client:Revive', targetId)
        return
    end
    
    -- ESX Ambulancejob
    if isStarted('esx_ambulancejob') then
        TriggerClientEvent('esx_ambulancejob:revive', targetId)
        return
    end
    
    -- ND Core
    if isStarted('ND_Core') then
        TriggerClientEvent('ND_Ambulance:revive', targetId)
        return
    end
    
    -- Fallback: Set player health directly via client
    TriggerClientEvent('lunar_unijob:forceRevive', targetId)
end
...
RegisterNetEvent('lunar_unijob:revivePlayer', function(targetId)
    ...
    triggerRevive(targetId)
end)

-- Universal heal function compatible with multiple ambulance scripts
local function triggerHeal(targetId)
    -- Wasabi Ambulance
    if isStarted('wasabi_ambulance') then
        TriggerClientEvent('wasabi_ambulance:heal', targetId)
        return
    end
    
    -- Brutal Ambulance
    if isStarted('brutal_ambulance') then
        TriggerClientEvent('brutal_ambulance:heal', targetId)
        return
    end
    
    -- Fallback: Use built-in heal
